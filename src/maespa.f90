PROGRAM maespa
    ...
    !=======================================================================================
    ! Copyright 2019 Remko Duursma, Belinda Medlyn, Mathias Christina, Guerric le Maire, Remi Vezy, Celine Blitz-Frayret.
    !---------------------------------------------------------------------------------------
    ...
    CALL CPU_TIME(START) ! CBF for chrono


    VTITLE = 'MAESPA: 0.1.1' ! Carefull, increments only using semver: https://semver.org/
...
!                ROOTLEN = ROOTLENSPEC(1:MAXSOILLAY,ISPEC)
                ...
                IF ((ABS(ZEN(IHOUR)) <  PI/2.0 ) .AND. (RADABV(IHOUR,1) > 1.0) .AND. (FOLT(1) > 0.0)) THEN
                    ...
                    IF(ISIMUS.EQ.1)THEN
                        ...
                        CALL SUMHRUS(IHOUR,NOUSPOINTS,GRDAREAI,PARUS,PARUSMEAN,PARUSSD,APARUS,PSUS,ETUS,THRABUS,&
                                    FCO2US,FH2OUS) ! CBF removed areas
...
!**********************************************************************
    ...
    CALL CPU_TIME(FINISH) ! CBF for chrono

    WRITE(*,*) 'Plot ', numpar,' Time = ',FINISH-START,' seconds'
...
!**********************************************************************
SUBROUTINE SUMHRUS(IHOUR,NOUSPOINTS,GRDAREAI,PARUS,PARUSMEAN,PARUSSD,&
                    APARUS,PSUS,ETUS,THRABUS,FCO2US,FH2OUS)! CBF removed areas
...
!**********************************************************************
    ...
    REAL PARUSMEAN(MAXHRS),PARUSSD(MAXHRS) ! CBF removed areaus(MAXP)
    REAL GRDAREAI
    REAL, EXTERNAL :: STDEV
